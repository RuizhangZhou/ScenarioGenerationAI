...
for n in range(33):
    ...
    with open("/DATA1/rzhou/ika/multi_testcases/%(Dtype)s_multi.csv" %{'Dtype':Dtype}, "a") as f_multi:
        for i in range(length):
            ...
            # tracks_np = pd.read_csv("/DATA1/rzhou/ika/%(Dtype)s/data/%(n)02d_tracks.csv" %{'Dtype':Dtype,'n':n},low_memory=False).values
            # rows_tracks_np = tracks_np.shape[0] 
            # with open("/DATA1/rzhou/ika/multi_testcases/%(Dtype)s_multi_23-32.csv" %{'Dtype':Dtype}, "a") as f_multi:
...
# nohup env CUDA_VISIBLE_DEVICES=0,1,2 python multi_case_improved.py >> /home/rzhou/Projects/scenariogenerationai/multi_log/multicase_inD.log 2>&1 &
